...
BEGIN
    FOR i IN 1..array_length(dist_list,1) LOOP
        EXECUTE format($f$
            ...
            -- Select target to join on intersect
            DROP TABLE IF EXISTS features_in_buffers_%1$s;

            CREATE TABLE features_in_buffers_%1$s AS
            SELECT f.*
            FROM target_features f
            JOIN overlap_3buffers_union_%1$s o
              ON ST_Intersects(f.shape, o.geom);
